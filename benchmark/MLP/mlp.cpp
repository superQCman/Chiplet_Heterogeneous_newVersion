...
void GpuMultiply(double* mat1,double* mat2,int fst_Row,int fst_Col,int sec_Row,int sec_Col,std::vector<std::vector<double>>& Res,int dstX,int dstY){
    ...
    Mat1_size[0]=fst_Row;Mat1_size[1]=fst_Col;
    Mat2_size[0]=sec_Row;Mat2_size[1]=sec_Col;
    DoubleToInt(mat1,Mat1,fst_Row*fst_Col);
    DoubleToInt(mat2,Mat2,sec_Row*sec_Col);
    sendGpuMessage(dstX,dstY,srcX,srcY,Mat1_size,2);
    std::cout<<"#############################################cpusendmessage"<<std::endl;
    sendGpuMessage(dstX,dstY,srcX,srcY,Mat2_size,2);
    char* fileName=new char[100];
    sprintf(fileName,"./gpuRead1%d_%d_%d_%d",srcX,srcY,dstX,dstY);
    mkfile(fileName);
    sendGpuMessage(dstX,dstY,srcX,srcY,Mat1,fst_Col*fst_Row);
    std::cout<<"#############################################cpusendmessage"<<std::endl;
    sendGpuMessage(dstX,dstY,srcX,srcY,Mat2,sec_Row * sec_Col);
    std::cout<<"#############################################cpusendmessage"<<std::endl;
    ...
    sprintf(fileName,"./cpuRead%d_%d_%d_%d",dstX,dstY,srcX,srcY);
    delfile(fileName);
    std::cout<<"#############################################test2"<<std::endl;
    double *result=new double[fst_Row*sec_Col];
    int64_t *Result_2=new int64_t[fst_Row*sec_Col];
    readGpuMessage(dstX,dstY,srcX,srcY,Result_2,fst_Row*sec_Col);
    std::cout<<"#############################################test3"<<std::endl;
    sprintf(fileName,"./buffer%d_%d_%d_%d",dstX,dstY,srcX,srcY);
    delfile(fileName);
...
void ToGPU(double* mat1,double* mat2,int fst_Row,int fst_Col,int sec_Row,int sec_Col,std::vector<std::vector<double>>& Res,int gpu_num){
    ...
    for(size_t i=0;i<dev1.size();i++){
        ...
        //std::thread t(&BPNeuralNetwork::GpuMultiply,this,Dev1,Dev2,dev1[i].size(),dev1[i][0].size(),dev2[i].size(),dev2[i][0].size(),std::ref(res[i]),dstX,i+1);
        THREAD.push_back(std::thread (GpuMultiply,Dev1,Dev2,dev1[i].size(),dev1[i][0].size(),dev2[i].size(),dev2[i][0].size(),std::ref(res[i]),dstX,i+1));
    }
    for(auto &i:THREAD){
        i.join();
    }
...
void backward(std::vector<std::vector<double>> x, std::vector<std::vector<double>>& y,double learning_rate){
    ...
    for(int i=weight.size()-1;i>=0;i--){
        //double* Deltas=new double[deltas[deltas.size()-1].size()*deltas[deltas.size()-1][0].size()];
        ...
        std::thread t1(ToGPU,Weight,Dz,weight[i][0].size(),weight[i].size(),dz.size(),dz[0].size(),std::ref(deltas_pre),1);
        //GpuMultiply(Weight,Dz,weight[i][0].size(),weight[i].size(),dz.size(),dz[0].size(),std::ref(deltas_pre),1);
        std::thread t2 (ToGPU,Dz,Activations_i,dz.size(),dz[0].size(),activations[i][0].size(),activations[i].size(),std::ref(dw),2);
        std::cout<<"***********************************************************************************"<<i<<std::endl;
...
void train(std::vector<std::vector<double>>& x,std::vector<std::vector<double>> y,int num_iterations=1000,double learning_rate=0.1){
...
bool readDataFromJSON(const std::string& filename, std::vector<std::vector<double>>& x_train, std::vector<std::vector<double>>& x_test,std::vector<std::vector<double>>& y_train,std::vector<std::vector<double>>& y_test) {
    ...
    for (const auto& val : data["y_train"]) {
        ...
        if(val==1){
            ...
            temp.push_back(0);temp.push_back(1);temp.push_back(0);
        }else{
            temp.push_back(0);temp.push_back(0);temp.push_back(1);
        }
...
int main(int argc,char** argv){
    ...
    hidden_size.push_back(10);hidden_size.push_back(15);
